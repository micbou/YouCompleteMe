...
than 7.4.1898). Furthermore, the `mods=` argument accepts two special modifiers
`curwin` and `existwin`. See [the `GoTo` commands](#goto-commands) for more
details.
...
You can control where the destination is opened by setting [the
`g:ycm_goto_buffer_command` option](#the-gycm_goto_buffer_command-option) to
`''` (for backward compatibility reason) and by using one of the following
command modifiers: `aboveleft`, `belowright`, `botright`, `leftabove`,
`rightbelow`, `topleft`, `vertical`, and `tab`. See `:h mods` for a description
of these modifiers. They can also be specified through the `mods=` argument as a
comma-separated list e.g. `mods=aboveleft,vertical`.
Furthermore, the `mods=` argument accepts two special modifiers:

- `curwin`: jump in the current window;
- `existwin`: if the result is already open in a window of the current tab
  page or any tab pages with the `tab` modifier, jump to that window.

The default behavior when `g:ycm_goto_buffer_command` is set to `''` is to jump
into a new window split horizontally above the current one.

Examples:

- jump in the current window:

  ```viml
  :YcmCompleter mods=curwin GoTo
  ```

- jump into a new window split vertically to the left of the current one:

  ```viml
  :aboveleft vertical YcmCompleter GoTo
  ```

  or (required for Vim older than 7.4.1898):

  ```viml
  :YcmCompleter mods=aboveleft,vertical GoTo
  ```

- jump into an existing window in the current tab; if no such window, jump in
  the current window:

  ```viml
  :YcmCompleter mods=curwin,existwin GoTo
  ```

- jump into an existing window in any tab pages; if no such window, jump to a
  new tabpage:

  ```viml
  :tab YcmCompleter mods=curwin,existwin GoTo
  ```

  or (required for Vim older than 7.4.1898):

  ```viml
  :YcmCompleter mods=curwin,existwin,tab GoTo
  ```
...
**NOTE:** this option is deprecated. You should set its value to `''` and use
command modifiers as a prefix of the command or as a comma-separated list
through the `mods=` argument. See [the `GoTo` commands](#goto-commands) for more
details. This will become the default behavior once the option is removed.
